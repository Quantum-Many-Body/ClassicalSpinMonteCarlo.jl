...
function Observables(lattice::T, fbasis::Bool) where T<:Lattice
    ...
    nsite = fbasis==true ? length(lattice.unitcell.basis) : lattice.length
    return Observables(ErrorPropagator(Float64), LogBinner(Float64), LogBinner(zeros(Float64,3)), LogBinner(zeros(Float64,lattice.length, nsite)), fbasis) 
end
...
function performMeasurements!(observables::Observables, lattice::T, energy::Float64) where T<:Lattice
    ...
    push!(observables.correlation, getCorrelation(lattice, observables.fbasis))
end