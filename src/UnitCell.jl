...
struct UnitCell{D}
	...
	interactions::Vector{Tuple{Vector{Int},Vector{NTuple{D,Int}},Array{Float64}}}
    ...
    UnitCell(a1::NTuple{1,Float64}) = new{1}((a1,), Vector{NTuple{1,Float64}}(undef,0), Vector{Tuple{Vector{Int},Vector{NTuple{1,Int}},Array{Float64}}}(undef,0))
    UnitCell(a1::NTuple{2,Float64}, a2::NTuple{2,Float64}) = new{2}((a1,a2), Vector{NTuple{2,Float64}}(undef,0), Vector{Tuple{Vector{Int},Vector{NTuple{2,Int}},Array{Float64}}}(undef,0))
    UnitCell(a1::NTuple{3,Float64}, a2::NTuple{3,Float64}, a3::NTuple{3,Float64}) = new{3}((a1,a2,a3), Vector{NTuple{3,Float64}}(undef,0), Vector{Tuple{Vector{Int},Vector{NTuple{3,Int}},Array{Float64}}}(undef,0))
    UnitCell(primitives...) = new{length(primitives)}(primitives, Vector{NTuple{length(primitives),Float64}}(undef,0), Vector{Tuple{Vector{Int},Vector{NTuple{length(primitives),Int}},Array{Float64}}}(undef,0))	
end
...
"""
function addInteraction!(unitcell::UnitCell{D}, bs::Vector{Int}, offset::Vector{NTuple{D,Int}}, M::Array{Float64,L}) where {D,L}
    size(M) == Tuple(3*ones(Int,L)) || error("Interaction matrix must be of size 3.")
    (length(bs)==L && length(offset) == L) || error("Length of the interaction must be the same.")

    push!(unitcell.interactions, (bs,offset,M))
end
